...
class VMToUserLayer(MessagePassing):
    def __init__(self, vm_hidden_dim, user_hidden_dim, edge_dim):
        super().__init__(aggr='add')
        self.input_norm = nn.LayerNorm(vm_hidden_dim + edge_dim + user_hidden_dim)
        self.mlp = nn.Sequential(
            nn.Linear(vm_hidden_dim + edge_dim + user_hidden_dim, user_hidden_dim),
            nn.ReLU(),
            nn.Linear(user_hidden_dim, user_hidden_dim),
            nn.ReLU()
        )
